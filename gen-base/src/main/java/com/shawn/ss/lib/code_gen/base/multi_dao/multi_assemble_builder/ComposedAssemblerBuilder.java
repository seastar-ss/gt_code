...
//    private AbstractJClass multiAsemmblerClz;
    ...
    public ComposedAssemblerBuilder(ModelMulDaoConf modelMulDaoConf, AbstractJClass wrapperCls) {
...
//        isList=modelMulDaoConf.
    ...
    public void buildModel() {
        try {
            ...
            CodeHelper.buildGetterAndSetter(definedClass,"ret",wrapperCls,ret);
            AbstractJClass wrapperListCls = CodeHelper.buildNarrowedClass(cm, List.class, wrapperCls);
            JFieldVar rets = definedClass.field(JMod.PROTECTED, wrapperListCls, "rets",cm.ref(CollectionHelper.class).staticInvoke("newList"));
            CodeHelper.buildGetterAndSetter(definedClass,"rets",wrapperListCls,rets);
            buildClearMethod();
            buildMainMethod();
    ...
    private void buildMainMethod() {
        ...
        JMethod method = definedClass.method(JMod.PUBLIC, cm.INT, CodeHelper.LIB_SQL_ASSEMBLE_SQL);

        JMethod assembleMain = definedClass.method(JMod.PROTECTED, cm.INT, CodeHelper.getMethodNameOfMultiDaoAssemble(mainField));
...
//            String serviceClassName = modelBuilderContext.getServiceClassName(modelMulDaoConf.getMainTable());
        ...
        staticRoundFieldVar[findex]=definedClass.field(
                ...
                CodeHelper.getFieldNameOfMulDaoRNDName(mainField),
                JExpr.lit(findex)
        );
        JBlock jBlock = body._if(JExpr.invoke(CodeHelper.METHOD_ASSEMBLER_GET_INDEX).eq(staticRoundFieldVar[0]))._then();
        jBlock._return(CodeHelper.invokeMethodWithParam(assembleMain,jVars));
...
//            for(ModelRelatedTableDef def:relatedTables){
//                String fieldName = def.getFieldName();
            JMethod subMethod = definedClass.method( JMod.PROTECTED, cm.INT, CodeHelper.getMethodNameOfMultiDaoAssemble(fieldName));
            addAssembleParam(subMethod,false);
            subMethod.body()._return(JExpr.lit(0));
            String fieldVarName= CodeHelper.getFieldNameOfMulDaoRNDName(fieldName);
            jBlock = body._if(JExpr.invoke(CodeHelper.METHOD_ASSEMBLER_GET_INDEX).eq(fieldVar))._then();
            jBlock._return(CodeHelper.invokeMethodWithParam(subMethod,jVars));
        }
...
//        List<JFieldVar> allVars= CollectionHelper.newList(3);
        ...
        JVar tClassVar = method.param(CodeHelper.buildNarrowedClass(cm, Class.class, cm.ref(AbstractBaseModel.class).wildcard(EWildcardBoundMode.EXTENDS)), "tClass");
