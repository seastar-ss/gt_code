...
function buildAll(tmpl, templates) {
    ...
    if (txt && txt.length > 0) {
        ...
        for (var name in templates) {
            ...
            item.name = name;
            item.script = baseHost + basePath + name+".js";
            item.css = baseHost + basePath + "css-"+name+".css";
            build(item);
            // print("")
            //this["$"+name]=item;
...
function commonSetting(base, pos, title, func) {
    ...
    var cass = __.lsAllByExtention(path.dirname(def.__body), ".scss");
    def.csses = def.csses.concat(less);
    def.csses = def.csses.concat(cass);
