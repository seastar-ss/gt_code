...
var res = function (ts) {
    var alltemplates={};
    for(var tmpl in ts.templates){
        var item=ts.templates[tmpl];
        ts.preBuild(item.tmpl,item.pages);
        for(var i in item.pages){
            alltemplates[i]=item.pages[i];
        }
    }
    var ret = {
        input: {},
    ...
    for (var x in alltemplates) {

        var plugins = [];
        var templateItem = alltemplates[x];
        ...
        var extractStyles = new ExtractTextPlugin({
            filename: (getPath) => {
                return getPath('css-'+x+'.css');
            },
            disable: process.env.NODE_ENV === "development",
            ignoreOrder: true
        });
        var extractHtmls = new ExtractTextPlugin({
            filename: (getPath) => {
                return getPath('html-'+x+'.html');
            },
            disable: process.env.NODE_ENV === "development"
        });
        var rules = [
            ...
            //    test: /^.*_img_[^_\.]+\.svg$/i,
            //    use: [
            //        'file-loader?name=../../asset/i[hash:32].[ext]', // or url-loader
            //        {
