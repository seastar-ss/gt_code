...
public class JFormatter implements Closeable
{
  ...
  void write (@Nonnull final JDefinedClass aClassToBeWritten)
  {
    ...
    Set<String> m_aImports = aClassToBeWritten.getM_aImports();
    if(m_aImports!=null && m_aImports.size()>0) {
      for (String clz : m_aImports) {
        print("import").print(clz).print(';').newline();
        bAnyImport = true;
      }
    }
